...
class PointMalSystem:
    ...
    def run_complete_av_agent(self, seqs: List[int], conn, output_dir) -> Dict:
        ...
        for seq in seqs:
            ...
            task = f"""
As a professional malware analysis expert, please make a judgment based on the preliminary detection result, APK xml configuration, code, and the features.
# Initial machine learning detection result: [{'Malicious' if float(template['prob'])>=0.5 else 'Benign'}] 
# Classifier Confidence Score [ {template['prob']}]
